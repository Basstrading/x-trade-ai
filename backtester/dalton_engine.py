...
class DaltonEngine:
    ...
    def __init__(self, params=None):
        ...
        # Stop ratios (proportion de l'IB range)
        ...
        # Limites
        self.daily_loss_limit = p.get('daily_loss_limit', -1800)
        self.max_trades_day = p.get('max_trades_day', 4)
        self.pause_after_stops = p.get('pause_after_stops', 2)

        # Flat filter — close all at this time
        self.close_hour = p.get('close_hour', 21)
        self.close_min = p.get('close_min', 44)

        # Point value et contrats
    ...
    def _dollar(self, pts):
        return pts * self.point_value * self.contracts
    ...
    def calc_value_area(self, df: pd.DataFrame) -> dict:
        ...
        if not va_prices:
            return {}
        return {'poc': float(poc), 'vah': float(max(va_prices)), 'val': float(min(va_prices))}

    # === OPEN TYPE ===

    def classify_open_type(self, open_price, prev_vah, prev_val, prev_poc, first_bars):
        if len(first_bars) < 3:
            return 'OTO'
        first = first_bars.iloc[:3]
        high_f = first['high'].max()
        low_f = first['low'].min()
        range_f = high_f - low_f
        close_f = first['close'].iloc[-1]
        move = close_f - open_price
        directional = abs(move) / max(range_f, 1)

        if directional > 0.6 and range_f > 0:
            return 'OTD_UP' if move > 0 else 'OTD_DOWN'
        if high_f > prev_vah and close_f < prev_vah:
            return 'OTR_DOWN'
        if low_f < prev_val and close_f > prev_val:
            return 'OTR_UP'
    ...
    def classify_day_type(self, ctx, current_bars):
    ...
    def get_signal(self, ctx, bar, prev_bar, current_hour, current_minute):
            ...
            if ctx.day_type in ['trend_up', 'double_dist_up', 'normal']:
                if 17 <= current_hour <= 18:
                    if bar['close'] > ctx.ib_high:
                        return {'direction': 'long', 'signal': 'otd_breakout',
                                'stop_ref': ctx.ib_low, 'target': 0,
            ...
            if ctx.day_type in ['trend_down', 'double_dist_down', 'normal']:
                if 17 <= current_hour <= 18:
                    if bar['close'] < ctx.ib_low:
                        return {'direction': 'short', 'signal': 'otd_breakout',
                                'stop_ref': ctx.ib_high, 'target': 0,
                                'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}

        # TREND PULLBACK 17h-21h
        if day_type == 'trend_up' and 17 <= current_hour < 21:
            if (prev_bar['low'] <= ctx.ib_high
                ...
                return {'direction': 'long', 'signal': 'trend_pullback',
                        'stop_ref': bar['low'], 'target': 0,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}
        ...
        if day_type == 'trend_down' and 17 <= current_hour < 21:
            if (prev_bar['high'] >= ctx.ib_low
                    ...
                    and bar['close'] < prev_bar['close']):
                return {'direction': 'short', 'signal': 'trend_pullback',
                        'stop_ref': bar['high'], 'target': 0,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}

        # NORMAL DAY — Fade IB 17h-21h
        if day_type in ['normal', 'unknown'] and 17 <= current_hour < 21:
            if (bar['high'] >= ctx.ib_high and bar['close'] < ctx.ib_high
                ...
                return {'direction': 'short', 'signal': 'fade_ib_high',
                        'stop_ref': bar['high'], 'target': poc_target,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}
            ...
            if (bar['low'] <= ctx.ib_low and bar['close'] > ctx.ib_low
                ...
                poc_target = ctx.prev_poc if ctx.prev_poc > 0 else (ctx.ib_high + ctx.ib_low) / 2
                return {'direction': 'long', 'signal': 'fade_ib_low',
                        'stop_ref': bar['low'], 'target': poc_target,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}

        # NEUTRAL DAY — Fade extrêmes
        if day_type == 'neutral' and 17 <= current_hour < 21:
            mid = (ctx.ib_high + ctx.ib_low) / 2
            if (bar['high'] >= ctx.ib_high and bar['close'] < ctx.ib_high
                    and abs(bar['close'] - ctx.ib_high) <= 8):
                return {'direction': 'short', 'signal': 'neutral_fade_high',
                        'stop_ref': bar['high'], 'target': mid,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}
            if (bar['low'] <= ctx.ib_low and bar['close'] > ctx.ib_low
                    and abs(bar['close'] - ctx.ib_low) <= 8):
                return {'direction': 'long', 'signal': 'neutral_fade_low',
                        'stop_ref': bar['low'], 'target': mid,
                        'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}

        # DOUBLE DIST BREAKOUT 17h30-21h
        if day_type == 'double_dist_up':
            if 17 <= current_hour < 21 and not (current_hour == 17 and current_minute < 30):
                if bar['close'] > ctx.ib_high and prev_bar['close'] <= ctx.ib_high:
                    return {'direction': 'long', 'signal': 'double_dist_break',
                            'stop_ref': ctx.ib_high, 'target': 0,
                            'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}
        ...
        if day_type == 'double_dist_down':
            if 17 <= current_hour < 21 and not (current_hour == 17 and current_minute < 30):
                if bar['close'] < ctx.ib_low and prev_bar['close'] >= ctx.ib_low:
                    return {'direction': 'short', 'signal': 'double_dist_break',
                            'stop_ref': ctx.ib_low, 'target': 0,
                            'ib_high': ctx.ib_high, 'ib_low': ctx.ib_low}
    ...
    def get_mm(self, day_type, direction, entry, signal, ib_range, ib_high, ib_low):
        ...
        if signal_type in ['fade_ib_high', 'neutral_fade_high'] and direction == 'short':
            buf = max(stop_pts * 0.3, 5)
            stop = sig_ib_high + buf
        elif signal_type in ['fade_ib_low', 'neutral_fade_low'] and direction == 'long':
            buf = max(stop_pts * 0.3, 5)
            stop = sig_ib_low - buf
        elif direction == 'long':
            stop = entry - stop_pts
        ...
        return {'stop': stop, 'target': target, 'trail_pts': trail_pts, 'stop_pts_used': round(stop_pts, 1)}

    # === MAIN LOOP ===

    def run(self, df_5min, daily_loss_limit=-1800, max_trades_per_day=4):
        ...
        for trade_date in sorted_dates:
            ...
            for bar_idx, (ts, bar) in enumerate(bars_of_day):
                ...
                # Flat filter
                force_flat = (hour > self.close_hour or (hour == self.close_hour and minute >= self.close_min))

                # Pause after consecutive stops
                ...
                if daily_pnl <= self.daily_loss_limit:
                    if in_trade:
                        ...
                        pnl_usd = self._dollar(pnl_pts)
                        equity += pnl_usd
                        daily_pnl += pnl_usd
                        self._record(all_trades, entry_time, ts, direction, entry_price,
                                     bar['close'], stop_price, pnl_pts, 'daily_limit',
                                     signal_type, ctx.day_type, ctx.open_type, bars_held,
                ...
                if trades_today >= self.max_trades_day and not in_trade:
                    continue
                ...
                if in_trade:
                    ...
                    target_hit = False
                    if target_price > 0:
                        target_hit = ((direction == 'long' and bar['high'] >= target_price)
                                      or (direction == 'short' and bar['low'] <= target_price))
                    if target_hit:
                        ...
                        consecutive_stops = 0
                        self._record(all_trades, entry_time, ts, direction, entry_price,
                                     target_price, stop_price, pnl_pts, 'target',
                                     signal_type, ctx.day_type, ctx.open_type, bars_held,
                                     target_price, stop_initial)
                        trades_today += 1
                        in_trade = False
                    ...
                    # Stop hit
                    stop_hit = ((direction == 'long' and bar['low'] <= stop_price)
                                or (direction == 'short' and bar['high'] >= stop_price))
                    if stop_hit:
                        exit_p = stop_price
                        ...
                        self._record(all_trades, entry_time, ts, direction, entry_price,
                                     exit_p, stop_price, pnl_pts, reason,
                                     signal_type, ctx.day_type, ctx.open_type, bars_held,
                                     target_price, stop_initial)
                        trades_today += 1
                        in_trade = False
                    ...
                    if target_price > 0 and not mid_stop_set:
                        ...
                        if mid_hit:
                            be = entry_price + 2 if direction == 'long' else entry_price - 2
                            if direction == 'long' and be > stop_price:
                                stop_price = be
                            elif direction == 'short' and be < stop_price:
                                stop_price = be
                            mid_stop_set = True

                    # FLAT FILTER — close at configured time
                    if force_flat:
                        pnl_pts = self._pnl(entry_price, bar['close'], direction)
                        pnl_usd = self._dollar(pnl_pts)
                        equity += pnl_usd
                        daily_pnl += pnl_usd
                        self._record(all_trades, entry_time, ts, direction, entry_price,
                                     bar['close'], stop_price, pnl_pts, 'flat',
                                     signal_type, ctx.day_type, ctx.open_type, bars_held,
                                     target_price, stop_initial)
                        trades_today += 1
                        in_trade = False
                ...
                # === SIGNAL SEARCH ===
                    ...
                    signal = self.get_signal(ctx, bar, prev_bar, hour, minute)
                    if signal is None:
                        continue

                    mm = self.get_mm(ctx.day_type, signal['direction'], bar['close'], signal,
            ...
            if in_trade:
                ...
                equity += pnl_usd
                daily_pnl += pnl_usd
                self._record(all_trades, entry_time, last_ts, direction, entry_price,
                             last_bar['close'], stop_price, pnl_pts, 'flat',
                             signal_type, ctx.day_type, ctx.open_type, bars_held,
                             target_price, stop_initial)
                in_trade = False
                equity_curve.append(equity)
        ...
        return self._generate_report(all_trades, equity, equity_curve, daily_pnl_dict)

    def _pnl(self, entry, exit_p, direction):
        return (exit_p - entry) if direction == 'long' else (entry - exit_p)

    def _dollar(self, pts):
        return pts * self.point_value * self.contracts

    def _record(self, trades, entry_time, exit_time, direction, entry, exit_p,
                stop, pnl_pts, reason, signal, day_type, open_type, bars_held,
                target, stop_initial):
        pnl_usd = self._dollar(pnl_pts)
        trades.append({
    ...
    def _generate_report(self, trades, equity, equity_curve, daily_pnl):
        if not trades:
            return None
        ...
        r.expectancy = round(np.mean(pnls), 2) if pnls else 0
        ...
        for t in trades:
            ...
            if reason == 'stop':
                ...
                r.exits_target += 1
            elif reason in ['flat', 'session_end']:
                r.exits_flat += 1
        ...
        if longs:
            r.long_winrate = round(len([t for t in longs if t['pnl_dollars'] > 0]) / len(longs) * 100, 1)
        if shorts:
            r.short_winrate = round(len([t for t in shorts if t['pnl_dollars'] > 0]) / len(shorts) * 100, 1)
        ...
        for sig in r.trades_by_signal:
            ...
            w = [t for t in st if t['pnl_dollars'] > 0]
            r.winrate_by_signal[sig] = round(len(w) / len(st) * 100, 1) if st else 0
        for dt in r.trades_by_daytype:
            dtt = [t for t in trades if t['day_type'] == dt]
            w = [t for t in dtt if t['pnl_dollars'] > 0]
            r.winrate_by_daytype[dt] = round(len(w) / len(dtt) * 100, 1) if dtt else 0
        ...
        if daily_pnl:
            ...
            r.days_over_agent_limit = sum(1 for p in pnl_vals if p < self.daily_loss_limit)
            r.days_over_topstep_limit = sum(1 for p in pnl_vals if p < -4500)
            r.avg_daily_pnl = round(np.mean(pnl_vals), 2)
            r.projected_monthly = round(r.avg_daily_pnl * 20, 2)
